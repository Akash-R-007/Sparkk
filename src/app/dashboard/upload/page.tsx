  ...
  const handleDeleteUploadedFile = async (file: UploadedFile) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setDeleteStatus(`❌ Failed to delete: ${result.detail || 'Unknown error'}`)
        return
      }
  ...
  const handleCreateAlpaca = async (file: UploadedFile) => {
    ...
    try {
      ...
      if (!res.ok || result?.error || result?.detail) {
        ...
        setUploadStatus(`❌ Gemini error: ${errMsg}`)
      } else {
        setAlpacaChunks(result.alpaca_format)
        setUploadStatus('✅ Alpaca format generated successfully!')

        // ✅ Save the Alpaca chunks as JSON to dataset directory
        const saveRes = await fetch('http://localhost:8000/save-alpaca-json', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        if (!saveRes.ok || saveResult?.error) {
          console.error('❌ Save error:', saveResult.error || 'Unknown error')
          setUploadStatus(`⚠️ Generated but not saved: ${saveResult.error || 'Unknown error'}`)
        } else {
          console.log('✅ Saved Alpaca JSON to:', saveResult.json_path)
          setUploadStatus('✅ Alpaca JSON saved to dataset folder!')
        }
      }
    } catch (err) {
        ...
        <Card>
          ...
          <CardContent>
            {alpacaChunks.map((chunk, index) => (
              <div
                key={index}
                className="border p-4 rounded-md bg-gray-50 mb-4 text-sm whitespace-pre-line"
              >
                <div className="font-semibold">Chunk {index + 1}</div>
                <div>Instruction: {chunk.instruction}</div>
                <div>Input: {chunk.input}</div>
                <div>Output: {chunk.output}</div>
              </div>
            ))}
          </CardContent>
        </Card>
